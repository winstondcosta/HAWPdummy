...
def train(cfg):
    ...
    n_dyn_junc = model.n_dyn_junc
    n_dyn_posl = model.n_dyn_posl
    n_dyn_negl = model.n_dyn_negl
    n_dyn_othr = model.n_dyn_othr
    n_dyn_othr2 = model.n_dyn_othr2
    n_pts1 = model.n_pts1
    dim_loi = model.dim_loi
    loss = model.loss
    fc2 = model.fc2
    pool1d = model.pool1d
    tspan = model.tspan
    ...
    ckpt_files = sorted([each for each in os.listdir(cfg.OUTPUT_DIR) if 'model' in each])
    arguments['epoch'] = 0
    if len(ckpt_files):
    ...
    for epoch in range(start_epoch + 1, arguments['max_epoch'] + 1):
        ...
        for it, (images, annotations) in enumerate(train_dataset):
            ...
            outputs, loi_features = model(images)
