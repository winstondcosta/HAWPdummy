...
from parsing.backbones import build_backbone#, HybridModel
# from detectron2.modeling.backbone import build_backbone as build_backbone_detecton2
...
class WireframeDetector(nn.Module):
    def __init__(self, cfg_hawp, cfg_det, network_type="hawp"):
        super(WireframeDetector, self).__init__()
        #self.hafm_encoder = HAFMencoder(cfg_hawp)

        self.network_type = network_type
        self.mask_network = None
        if self.network_type == 'hawp':
            self.backbone = build_backbone(cfg_hawp)
        elif self.network_type == 'resnet_hawp':
            self.backbone = build_backbone_detecton2(cfg_det)
            self.fc = self._make_fc(256, 256)
            self.score = MultitaskHead(256, 9, head_size=[[3], [1], [1], [2], [2]])
        elif self.network_type == 'resnet_mask':
            self.backbone = build_backbone_detecton2(cfg_det) #HybridModel(cfg_det)
            self.mask_network = HybridModel(cfg_det, self.backbone)
        elif self.network_type == 'hg_mask':
            self.backbone = build_backbone(cfg_hawp)
            self.mask_network = HybridModel(cfg_det, self.backbone) 
        else:
            assert False, "Wrong network type input"

        self.n_dyn_junc = cfg_hawp.MODEL.PARSING_HEAD.N_DYN_JUNC
        self.n_dyn_posl = cfg_hawp.MODEL.PARSING_HEAD.N_DYN_POSL
        self.n_dyn_negl = cfg_hawp.MODEL.PARSING_HEAD.N_DYN_NEGL
        self.n_dyn_othr = cfg_hawp.MODEL.PARSING_HEAD.N_DYN_OTHR
        self.n_dyn_othr2 = cfg_hawp.MODEL.PARSING_HEAD.N_DYN_OTHR2
        self.n_pts0 = cfg_hawp.MODEL.PARSING_HEAD.N_PTS0
        self.n_pts1 = cfg_hawp.MODEL.PARSING_HEAD.N_PTS1
        self.dim_loi = cfg_hawp.MODEL.PARSING_HEAD.DIM_LOI
        self.dim_fc = cfg_hawp.MODEL.PARSING_HEAD.DIM_FC
        self.n_out_junc = cfg_hawp.MODEL.PARSING_HEAD.N_OUT_JUNC
        self.n_out_line = cfg_hawp.MODEL.PARSING_HEAD.N_OUT_LINE
        self.use_residual = cfg_hawp.MODEL.PARSING_HEAD.USE_RESIDUAL
        # self.
        if(torch.cuda.is_available()):
            self.register_buffer('tspan', torch.linspace(0, 1, self.n_pts0)[None, None, :].cuda())
        else:
            self.register_buffer('tspan', torch.linspace(0, 1, self.n_pts0)[None, None, :].cpu())
    ...
    def forward_test(self, images, annotations=None):
        ...
        if(self.network_type == "hawp"):
            mask_network, outputs, features = self.backbone(images, None)
        elif(self.network_type == "resnet_hawp"):
            image_data = preprocess_image(images)
            features, mask_network = self.backbone(image_data.tensor), None
            y = self.fc(features['p2'])
            outputs, features = [self.score(y)], y
        elif(self.network_type == "resnet_mask"):
            image_data = preprocess_image(images)
            features = self.backbone(image_data.tensor)
            mask_network, outputs, features = self.mask_network(image_data, features, [ann['mask_anns'] for ann in annotations])
        elif(self.network_type == "hg_mask"):
            _, outputs, features = self.backbone(images, None)
            features = {'p2' : features}
            mask_network, _, _ = self.mask_network(image_data, features, [ann['mask_anns'] for ann in annotations])
        ...
        if(mask_network and False):
            ...
            for polygon in polygons:
                ...
                for i in range(n):
                    ...
                    lines_new.append([x1/4, y1/4, x2/4, y2/4])
                    jloc_pred[0][0,y1//4, x1//4] = min(1.0, 0.1 + jloc_pred[0][0,y1//4, x1//4])
            lines_new = torch.tensor(lines_new).cuda()
            lines_pred = torch.cat([lines_pred, lines_new], dim = 0)
        ...
        return mask_network, output, extra_info
    ...
    def forward_train(self, images):
        ...
        if targets is not None:
            for nstack, output in enumerate(outputs):
                loss_map = torch.mean(F.l1_loss(output[:, :3].sigmoid(), targets['md'], reduction='none'), dim=1,
                                      keepdim=True)
        ...
        return loss_dict, loi_features, md_pred, dis_pred, res_pred, jloc_pred, joff_pred, self.tspan, self.pool1d, self.fc2
        '''
        '''
        lines_batch = []
        extra_info = {
        ...
        for i, (md_pred_per_im, dis_pred_per_im, res_pred_per_im, meta) in enumerate(
            ...
            juncs_pred, _ = get_junctions(non_maximum_suppression(jloc_pred[i]), joff_pred[i],
                                          topk=min(N * 2 + 2, self.n_dyn_junc))
...
def get_hawp_model(pretrained=False, cfg_det = None, network_type = "hawp"):
    from parsing.config import cfg
    import os
    model = WireframeDetector(cfg, cfg_det, network_type)
    if pretrained:
